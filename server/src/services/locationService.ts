...
const getNearbyCafes = async ({latitude, longitude, radius}: PlacesNearbySearchRequest): Promise<Place[]> => {
  ...
  const parsedResponse = placesNearbySearchResponseSchema.parse(axiosResponse.data);
